...
def edit_inventory(action: str, data: Dict[str, Any]) -> str:
    ...
    try:
        if action == "save":
            ...
            if "name" not in data:
                return "Error: 'name' is mandatory for delete"
...
def edit_knowledge(
    action: str, host: str, name: str, steps: List[str] = None, description: str = ""
) -> str:
    conn = sqlite3.connect(INFRA_DB)
    try:
        if action == "save":
            ...
            conn.execute(
                "DELETE FROM olt_knowledge WHERE host = ? AND syntax = ?", (host, name)
            )
            conn.commit()
