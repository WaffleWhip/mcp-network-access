        ...
        while (time.time() - start_time) < timeout:
            try:
                ...
                if chunk:
                    ...
                    SESSIONS[key]["rolling_buffer"] = (SESSIONS[key].get("rolling_buffer", "") + chunk)[-5000:]
                    last_data_time = time.time()
                else:
                    break
...
async def send_command(host, command=None, commands=None):
    key = host.upper()
    if key not in SESSIONS: return None, "NOT_CONNECTED"
    r, w = SESSIONS[key]["r"], SESSIONS[key]["w"]
    out = ""
    items = commands if commands else ([command] if command else [])
    btn_map = {"[SPACE]": " ", "[ENTER]": "\n", "[Q]": "q", "[QUIT]": "q", "[ESC]": "\x1B", "[TAB]": "\t"}
    
    for item in items:
        curr_buffer = SESSIONS[key].get("rolling_buffer", "").strip()
        if str(item).upper() not in btn_map and not curr_buffer.endswith(':'):
            w.write("\n"); await w.drain()
            await asyncio.sleep(0.1)
            await _read_until_silence(host, timeout=0.5)
        
        char = btn_map[str(item).upper()] if str(item).upper() in btn_map else (f"{item}" if item == "?" else f"{item}\n")
        w.write(char); await w.drain()
...
def get_last_buffer(host: str, lines: Optional[int] = None):
    ...
    limit = lines if lines is not None else SESSIONS.get(key, {}).get("preferred_length", 20)
...
def is_logged_in(host: str): return host.upper() in SESSIONS
def close_session(host: str):
    key = host.upper()
    if key in SESSIONS:
        try: SESSIONS[key]["w"].close()
        except: pass
        del SESSIONS[key]
...
async def _heartbeat():
    while True:
        ...
        for k, v in list(SESSIONS.items()):
            if (time.time() - v["last_activity"]) > 300: close_session(k)
            else:
                try: v["w"].write("\n"); await v["w"].drain()
                except: close_session(k)
